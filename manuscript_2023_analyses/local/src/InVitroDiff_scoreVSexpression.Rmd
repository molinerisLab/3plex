...
z$staminal<-as.factor(z$staminal)
z$ssRNA<-as.factor(z$ssRNA)
z$condition<-as.factor(z$condition)
z$secondary_structure<-as.factor(z$secondary_structure)
z$filter_repeat<-as.factor(z$filter_repeat)
z$cCRE_type<-as.factor(z$cCRE_type)

z %>% filter(cCRE_type=="dELS" & condition=="NPC_H9") -> z


ggplot(data=z, aes(x=reorder(ssRNA,-TPXcCRE_score), y=TPXcCRE_score, fill=mark_seqc))+geom_col()+facet_grid(condition+cCRE_type+score_type~min_length+guanine_rate,scales="free_x")+theme_bw()+theme(text =element_text(size=11),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+geom_hline(yintercept=log10(0.05),linetype="dashed")+geom_hline(yintercept=-log10(0.05), linetype="dashed")+geom_hline(yintercept=0, linetype="dotted")
