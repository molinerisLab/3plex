...
option_list <- list(
	...
	make_option(c("-l", "--level-name"), action="store", default=NULL, dest="level_name", help="Name of the level column used to split the table [deafult:\"None\"]"),
...
if(!is.null(level_name)){
	...
	for (l1 in level_name) {
	  for (p1 in predictors) {
		...
		if(l1>l2 & p1>p2) {
		  roc1<-roc_matrix[[l1]][[p1]]
		  roc2<-roc_matrix[[l2]][[p2]]
