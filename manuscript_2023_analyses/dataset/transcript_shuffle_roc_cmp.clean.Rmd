...
# TF Average ROC curves
```{r tf.roc}
tf <- read.delim("v8.10_TFBS/ALL_TFs.fimo_score.matrix.header_added.gz", header = T)
tf <- split(tf,tf$Gene_ID)
tf.roc <- lapply(tf, function(x){
  roc.list(x,"neg_pos","score")
})
print(plot.roc(roc.list = lapply(tf.roc,"[[","score"), smoothing = "binormal"))
```


```{r}
avg.tf <- read.delim("avg_roc_pre.tsv", header=T)
...
avg.tf <- rbind(avg.tf, c(0,1,0,1,1,1,1,1,1,1))
avg.tf <- rbind(avg.tf, c(1,0,1,0,0,0,0,0,0,0))
