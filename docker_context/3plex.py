...
parser.add_argument("out_dir", metavar="out_dir", help="The output directory. Out_dir should already exists. Pay attention to mount appropriate volumes if you are using docker (e.g. `docker run --rm -v $(PWD):$(PWD) 3plex:v0.0.1 ssRNA.fa dsDNA.fa $(PWD)` sould work as expected, `docker run --rm -v $(PWD):$(PWD) 3plex:v0.0.1 ssRNA.fa dsDNA.fa .` should not)")
parser.add_argument("--bed", metavar="dsDNA.bed", dest="dsDNA_bed", type=str, default=None, help="Genomic coordiantes of the DNA sequences in bed format. The 4th column must contain the same identifiers of the sequences in dsDNA.fa")
