...
parser.add_argument("dsDNA", metavar="dsDNA.fa", help="The DNA sequences in multi-FASTA format.")
parser.add_argument("out_dir", metavar="out_dir", help="The output directory. Out_dir should already exists. Pay attention to mount appropriate volumes if you are using docker (e.g. `docker run --rm -v $(PWD):$(PWD) 3plex:v0.0.1 ssRNA.fa dsDNA.fa $(PWD)` should work as expected, `docker run --rm -v $(PWD):$(PWD) 3plex:v0.0.1 ssRNA.fa dsDNA.fa .` should not)")
parser.add_argument("--bed", metavar="dsDNA.bed", dest="dsDNA_bed", type=str, default=None, help="Genomic coordinates of the DNA sequences in BED format. The 4th column must contain the same identifiers of the sequences in dsDNA.fa")
parser.add_argument("-j", "--jobs", metavar="CPUS", dest="jobs", type=int, default=1, help="Number of parallel threads.")
parser.add_argument("-l", "--min_length", metavar="N", dest="triplexator_min_length", type=int, default=8, help="Minimum triplex length required. Allowed values: N>5.")
parser.add_argument("-e", "--error_rate", metavar="E", dest="triplexator_error_rate", type=int, default=20, choices=range(100), help="Maximum percentage of error allowed in a triplex.")
...
parser.add_argument("-c", "--consecutive_errors", metavar="C", dest="triplexator_consecutive_errors", type=int, default=1, help="Maximum number of consecutive errors allowed in a triplex.")
parser.add_argument("-g", "--guanine_rate", metavar="G", dest="triplexator_guanine_rate", type=int, default=40, choices=range(100), help="Minimum percentage of guanines required in a TTS.")
...
parser.add_argument("-t", "--triplexator_other_parameters", metavar="T", dest="triplexator_other_parameters", type=str, default="", help="Additional triplexator parameters passed as a sting (e.g. -t '-mamg 90 -E 4'). 3plex output format will not change.")
...
parser.add_argument("--RNA2D_out", metavar="PATH", dest="RNA2D_out", type=str, default=None, help="Output the RNAplfold modified z-score in this PATH.")
parser.add_argument("--snakefile", metavar="file", dest="snakefile", type=str, default="/opt/3plex/Snakefile", help="Override the default Snakefile using the one specified.")
